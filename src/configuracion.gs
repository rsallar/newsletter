...
/**
 * FIX: Nueva función interna que SOLO obtiene los datos de las newsletters.
 * Se ha extraído la lógica de getNewslettersWithData para que el trigger
 * pueda obtener los datos sin volver a configurar los triggers, lo que causaba el problema.
 * @returns {Array<Object>} Un array de objetos, cada uno representando una newsletter.
 */
function _getAllNewsletters() {
...
/**
 * Obtiene todas las newsletters para la interfaz.
 * Esta función TAMBIÉN se asegura de que el trigger principal esté configurado.
 * @returns {Array<Object>} Un array de objetos, cada uno representando una newsletter.
 */
function getNewslettersWithData() {
  setupMainTrigger(); // Asegura que el trigger principal exista al cargar la app.
  return _getAllNewsletters();
}
...
/**
 * FIX: Lógica mejorada para ser 'idempotente'.
 * Ahora comprueba si el trigger ya existe antes de crearlo.
 * Esto evita eliminar y crear el trigger innecesariamente cada vez que se carga la app,
 * haciéndolo más eficiente y robusto.
 */
function setupMainTrigger() {
  const triggers = ScriptApp.getProjectTriggers();
  const triggerExists = triggers.some(
    trigger => trigger.getHandlerFunction() === MASTER_TRIGGER_HANDLER
  );

  if (!triggerExists) {
    // Por seguridad, borramos todos los triggers existentes antes de crear el nuestro.
    // Esto limpia el proyecto de triggers antiguos o duplicados.
    ...
    console.log(`El trigger principal '${MASTER_TRIGGER_HANDLER}' ya existe. No se realizaron cambios.`);
  }
}
